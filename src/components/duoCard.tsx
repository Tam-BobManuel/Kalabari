...
const DuoCard: React.FC<DuoCardProps> = ({ text, imageUrl, mobileImageUrl, linkUrl, linkText, heading, reverse }) => {
  ...
  return (
    <motion.div
      ...
      className={`md:mt-4 mt-2 justify-center flex items-center w-full contain border-box overflow-hidden lg:flex-row flex-col`}
    >
      <div className={`${reverse ? 'lg:flex-row-reverse' : ''} pt-0 px-2 flex mb-[20px] h-[3/4] overflow-none lg:flex-row flex-col`}>
        <div className={`lg:w-[60%] w-full h-full container ${reverse ? 'lg:ml-5' : 'lg:mr-5'} inline justify-self-stretch overflow-hidden bg-white bg-opacity-[4%] p-5`}>
        ...
        <div className="lg:w-[40%] lg:max-h-[450px] w-full h-full inline justify-self-stretch overflow-hidden">
          <Image src={imageUrl} alt="some Images on the Kalabari culture" objectFit='cover' width={1665} height={100} className="lg:block hidden" />
          <div className={'lg:hidden block h-full p-[20%] flex items-center justify-center w-full'} style={mobileBgImg}>
